...
const Incomes = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        "https://localhost:7007/api/Income/getAllForOneUserByEmail",
        {
          params: { email },
      ...
      const cleanedCategories = response.data.map((category) => ({
        ...
        expenseItems: category.incomeItems || [], // Заменяем expenseItems на incomeItems
        totalAmount: category.incomeItems.reduce(
  ...
  const fetchChartData = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCat) {
        const filteredItems = selectedCat.incomeItems.filter((item) => {
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://localhost:7007/api/Income/createCategory",
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      console.error("Ошибка при создании категории доходов:", error);
    }
  };

  const handleCreateIncomeItem = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://localhost:7007/api/Income/createIncomeItem?userId=${userId}`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      console.error("Ошибка при создании элемента дохода:", error);
    }
  };
  ...
  const handleUpdateCategory = async (isSubcategory = false) => {
    ...
    try {
      ...
      const url = isSubcategory
        ? `https://localhost:7007/api/Income/updateItem/${editingCategory.id}`
        : `https://localhost:7007/api/Income/updateCategory/${editingCategory.id}`;
  ...
  const handleDeleteCategory = async (id, isSubcategory = false) => {
    try {
      ...
      const url = isSubcategory
        ? `https://localhost:7007/api/Income/deleteItem/${id}`
        : `https://localhost:7007/api/Income/deleteCategory/${id}`;
      ...
      >
        ...
        <Dialog open={openAddDialog} onClose={() => setOpenAddDialog(false)}>
          ...
          <DialogActions>
            ...
            <Button onClick={parentId ? handleCreateIncomeItem : handleCreateCategory}>
              Создать
            </Button>
          </DialogActions>
        </Dialog>
        ...
        {categories.length === 0 && (
          <Typography variant="body1" sx={{ mt: 2 }}>
            Нет категорий доходов для этого пользователя.
          </Typography>
        )}
      </Box>
          ...
          <Grid item xs={12}>
            <Box>
              ...
              <Typography variant="body2" align="center" sx={{ mt: 2 }}>
                Общий доход за период: {totalBudget} руб.
              </Typography>
            </Box>
          </Grid>
        </Grid>
