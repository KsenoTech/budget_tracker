    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                ...
                const response = await axios.get('https://localhost:7007/api/ExpenseCategory/getAllForOneUserByEmail', {
                    params: { email },
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }, // Если требуется токен
                });
